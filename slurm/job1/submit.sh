...
#SBATCH --workdir=/working/wd15/git/Pace_visualization/slurm/job1  # -D, full path to an existing directory
#SBATCH --qos=test
#SBATCH --mem=0G
...
for n_chunk in 10 #20 40 80 120 160 200 240  
do
  for repeat in {1..2}
  do
    for samples in 1000 #8000
    do
      mfilename=mprof.dat
      ~/bin/nix-root nix-shell --pure --command "
